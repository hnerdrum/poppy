...
module Poppy.Codegen.JoinAliasSpec
  ( joinAliasSpec,
  )
where

import Poppy.Codegen.IR
import Poppy.Codegen.JoinAlias (IncludeAliases (..), assignIncludeAliases, assignModelAliases)
import Poppy.Codegen.Spec.Shelf (shelfSchema)
import Test.Hspec

joinAliasSpec :: Spec
joinAliasSpec =
  describe "Poppy.Codegen.JoinAlias" $ do
