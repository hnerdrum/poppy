...
import Poppy.Codegen.Emit.Schema (emitBelongsTo, emitHasMany, emitModelModule)
...
emitSpec = do
  describe "Poppy.Codegen.Emit.Schema" $ do
    ...
    it "includes HasMany in a Shelf model module" $ do
      ...
      actual `shouldSatisfy` T.isInfixOf "import Poppy.Relation (HasMany (..), JoinType (..))"
    ...
    it "includes BelongsTo in a Post model module" $ do
      ...
      actual
        `shouldSatisfy` T.isInfixOf "import Poppy.Relation (BelongsTo (..), JoinType (..))"
      actual
  ...
  describe "Poppy.Codegen.Emit.Schema.emitHasMany" $ do
  ...
  describe "Poppy.Codegen.Emit.Schema.emitBelongsTo" $ do
    it "emits author" $ do
      T.strip (emitBelongsTo authorPostSchema authorRel)
