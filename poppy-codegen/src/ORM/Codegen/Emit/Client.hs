...
module ORM.Codegen.Emit.Client
...
import ORM.Codegen.Emit.Combinator (combinatorImportItems, rootCombinatorTrees)
...
import ORM.Codegen.Lookup (lookupField, lookupModel, lookupRelation, lookupUniques)
  ...
  T.unlines
    ( [ "{-# LANGUAGE AllowAmbiguousTypes #-}",
        ...
        ++ [ "import Data.UUID (UUID)",
             ...
             "import ORM.PG (toField)",
             "import ORM.Core (fieldColumn)",
             "import ORM.Db (Db, liftIO, transaction)",
             "import qualified ORM.Delete as Delete",
             "import ORM.Errors (ORMError (..), requireFound)",
...
nestedEnumImports :: Schema -> Model -> [Text]
nestedEnumImports schema child =
  nubImports
    [ "import ORM.Schema." <> enumName e <> " (" <> enumName e <> " (..))"
      | f <- modelFields child,
...
clientSchemaPrefix clientModule =
  case T.stripPrefix "ORM.Client." clientModule of
    Just _ -> "ORM.Schema."
    _ -> "Schema."
