...
import ORM.Codegen.Emit.Include (emitIncludeModule)
import ORM.Codegen.Emit.Schema (emitEnumModule, emitModelModule)
...
genGolden file text =
  GenOutput
    { outputPath = "test/ORM/Codegen/golden/" <> file <> ".golden",
      outputText = text
    }
